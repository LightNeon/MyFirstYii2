...
function showRenameFileForm(id, event) {
    let titleEditor = $('#yii2-file-title-editor');
    let blockId = '#yii2-file-object-' + id;
    let title = $(blockId).attr('title');

    let block = $(document).find(blockId),
        leftPosition = block.offset().left,
        topPosition = block.offset().top + block.height();

    if (titleEditor.width() + leftPosition > $(window).width()) {
        const diff = (titleEditor.width() + leftPosition) - $(window).width();
        leftPosition = leftPosition - diff;
    }

    currentRenamingFileId = id;
    titleEditor.css('top', topPosition).css('left', leftPosition).fadeIn(100);
    titleEditor.find('input').val(title).focus();
}
