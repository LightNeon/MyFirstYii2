...
//    }
    ...
    public function getWatermark()
    {
        if (class_exists($this->class)) {
            if ($target = (new $this->class)::findOne($this->object_id)) {
                if (
                    isset($target->behaviors['files']) &&
                    isset($target->behaviors['files']->attributes[$this->field]) &&
                    isset($target->behaviors['files']->attributes[$this->field]['watermark'])
                )
                    return $target->behaviors['files']->attributes[$this->field]['watermark'];
            }
        }
    ...
    public function getPreviewWebPath(int $width = 0, bool $webp = false)
    {
        ...
        if (Yii::$app->getModule('files')->hostStatic)
            return
                ...
                "?hash={$this->hash}&width={$width}&webp=" . (int)$webp;
