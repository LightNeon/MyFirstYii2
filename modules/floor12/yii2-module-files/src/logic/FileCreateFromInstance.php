...
class FileCreateFromInstance
{
    ...
    public function __construct(UploadedFile $file, array $data, IdentityInterface $identity = null, $onlyUploaded = true)
    {
        ...
        $this->_instance = $file;
        $file->type = FileHelper::getMimeType($this->_instance->tempName);
    ...
    protected function setWatermarkAfterUpload()
    {
        $image = new SimpleImage();
        $image->load($this->_fullPath);
        $image->watermark(Yii::getAlias("@frontend/web/img/watermark.png"));
        $image->save($this->_fullPath);
    }
}
